...
async def dashboard_ui():
    ...
    try:
        ...
        meetings_response = supabase.table('meetings').select('id, title, display_title, meeting_date, duration_minutes, created_at, company, processing_status, meeting_metadata').order('created_at', desc=True).limit(50).execute()
        ...
        # Format meetings as tuples for template (use display_title if available)
        meetings = [(m['id'], m.get('display_title') or m['title'], m.get('meeting_date'), m.get('duration_minutes'), m.get('created_at'), m.get('company')) for m in meetings_data]
    ...
    <div class="main-content">
        ...
        <div class="container">
            <div class="nav-tabs">
                ...
                <button class="nav-tab" onclick="showSection('meetings')">Meetings</button>
                <button class="nav-tab" onclick="showSection('decisions')">Decisions</button>
                <button class="nav-tab" onclick="showSection('action-items')">Action Items</button>
                <button class="nav-tab" onclick="showSection('attendees')">Attendees</button>
            </div>
            <a href="/upload-ui" style="position: absolute; top: 32px; right: 32px; padding: 8px 12px; background: var(--gray-900); color: white; border-radius: 6px; text-decoration: none; font-size: 13px; font-weight: 500;">
            📤 Upload Files
        </a>
    </div>
        ...
        <div id="overview" class="section active">
            ...
            <div class="card" style="margin-bottom: 20px;">
                <div class="card-header">
                    <div class="card-title">📅 {m.get('display_title') or m['title'] or "Untitled Meeting"}</div>
