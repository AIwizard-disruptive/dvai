...
async def create_linear_from_meeting(meeting_id: str):
    ...
    For meeting: "Veckomöte - Team Meeting"
    Creates:
...
"""
        ...
        for idx, action in enumerate(actions, 1):
            try:
                ...
                if task:
                    ...
                    print(f"   ✓ [{idx}/{len(actions)}] {task['identifier']}: {action['title']} → {action.get('owner_name', 'Unassigned')}")
        ...
        print("\n" + "=" * 80)
        print("✅ LINEAR PROJECT & TASKS CREATED")
        print("=" * 80)
        print(f"\n📊 Summary:")
        print(f"  ✓ Project: {linear_project['name']}")
        print(f"  ✓ Tasks created: {len(results['linear_tasks'])}/{len(actions)}")
...
async def linear_sync_ui():
    ...
    for meeting in meetings.data:
        ...
        status_badge = '<span style="background: #4caf50; color: white; padding: 4px 12px; border-radius: 4px; font-size: 12px;">✓ Synced</span>' if has_project else '<span style="background: #ff9800; color: white; padding: 4px 12px; border-radius: 4px; font-size: 12px;">Not Synced</span>'
...
<body>
    ...
    <script>
        async function syncMeeting(meetingId, meetingTitle) {{
            ...
            resultDiv.innerHTML = `<h3>⏳ Creating Linear project and tasks...</h3><p>Meeting: ${{meetingTitle}}</p>`;
            ...
            try {{
                ...
                if (data.success) {{
                    ...
                    resultDiv.innerHTML = `
                        ...
                        <p><a href="${{data.results.linear_project.url}}" target="_blank" style="color: #0066cc; font-weight: 600;">📊 Open in Linear</a></p>
                        <hr>
                        <details>
                            <summary style="cursor: pointer; color: #0066cc;">Show all tasks</summary>
                            <ul style="margin-top: 12px;">
                                ${{data.results.linear_tasks.map(t => 
                                    `<li><strong>${{t.identifier}}</strong>: ${{t.title}} → ${{t.assignee}}</li>`
                                ).join('')}}
                            </ul>
                        </details>
                    `;
...
async def get_swedish_meeting_for_sync():
    ...
    meetings = supabase.table('meetings').select('id, title, meeting_date').ilike('title', '%Veckomöte%').execute()
